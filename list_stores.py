...
def scrape_tournaments(page, config):
    ...
    # Likely a dropdown or title-select screen on first load.
    try:
    ...
    # PLACEHOLDER selector — replace '.event-card' with the real class/structure.
...
def main():
    ...
    for t in tournaments:
        if wanted_stores and t["store"].strip().lower() not in wanted_stores:
            continue  # not one of your stores, skip
